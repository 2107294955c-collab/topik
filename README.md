...
- 语法笔记、搜索、等级筛选和复习记录；现有 15 条核心笔记之外另内置 400 条中高级语法，并提供到期/答错/随机范围的中韩双向主动回忆测验
- TOPIK II 51–54 题写作题库、独立分题草稿、可续做的全科计时、改写历史，以及 OpenAI 结构化分项评分、中文错误档案、韩语修订版和四题自动汇总
...
- 两种答题方式：考试计时整套模拟，以及逐题即时判分精练；桌面端支持数字/字母、方向键、收藏和标记快捷键
- 300 分全科模考流程：听力 60 分钟、写作 50 分钟、阅读 70 分钟，自动汇总分数并预测等级
- 支持整届模拟、随机 10 题、按薄弱题型专项训练、永久收藏题和错题重练
- 整套模拟支持答题卡、自由跳题、题目标记、退出续做、自动交卷、成绩统计与逐题复盘
- 在线阅读练习包含中文解题提示、来源状态和自动成绩记录
- 自动错题本、次日到期复习、连续答对计数、两次答对后自动掌握，以及错因标签、个人笔记和原因统计
- 真题完成状态、分数、正确率、错题笔记和复习日期
- 最近七天学习统计、记忆复习状态、数据质量提示、逐题速度分析、51–54 题型能力图和真题/全科模考成绩历史
- 自动统计阅读题型正确率，并从进度页一键进入最弱题型训练
- v12 JSON 数据备份、导入、合并及旧版数据迁移，并提供每 7 天备份健康提醒
- 手机、iPad、桌面响应式布局、主屏幕安装引导和轻量离线应用壳（PDF 不自动离线缓存）
...
- 内置词库、语法和真题不会重复写入 localStorage；浏览器只保存学习进度、自建内容和编辑内容。旧版完整数据会自动无损压缩，JSON 导出仍包含全部内容。
...
- 当前在线练习已实现阅读和听力四选一。听力录音不存放在本站，适合在另一台设备播放；写作可使用 OpenAI API 进行结构化评分，但密钥仍只保留在当前页面内存中。
