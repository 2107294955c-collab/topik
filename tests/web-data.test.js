...
assert.ok(window.TopikData.writingPrompts.filter(prompt => prompt.type === '54').every(prompt => /[가-힣]/.test(prompt.title) && /[가-힣]/.test(prompt.prompt)));
...
assert.ok(indexHtml.includes('20260819-4'));
...
assert.equal(version.version, '20260819-4');
