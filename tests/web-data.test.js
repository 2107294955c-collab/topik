...
assert.ok(appJs.includes('data-coverage-scope'));
assert.ok(appJs.includes('data-coverage-pdf'));
assert.ok(appJs.includes("const exam=forcedConfig?.examNumber||$('#practiceExamSelect').value"));
assert.ok(appJs.includes('const exam=forcedConfig?.examNumber'));
assert.ok(appJs.includes('state.activePractice.examNumber===exam&&state.activePractice.section===selectedSection&&state.activePractice.scope===scope'));
assert.ok(appJs.includes('function paperUrl(name){return `resources/past-papers/${encodeURIComponent(name)}`}'));
assert.ok(!appJs.includes('raw.githubusercontent.com/2107294955c-collab/topik'));
assert.ok(indexHtml.includes('id="essayGuidePanel"'));
assert.ok(indexHtml.includes('20260819-9'));
...
assert.equal(version.version, '20260819-9');
